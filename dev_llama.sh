#!/bin/bash
# Continuous development script for RustOS using llama.cpp
# llama.cpp provides reliable inference with full user control
...
LLAMA_MODEL_PATH="/home/k/models/qwen2.5-coder-7b-instruct-q8_0.gguf"
...
print_stats() {
    ...
    echo "  llama.cpp restarts:     $STAT_LLAMA_RESTARTS"
    ...
    log "INFO" "llama.cpp restarts: $STAT_LLAMA_RESTARTS, Model failures: $STAT_MODEL_LOAD_FAILURES"
...
# === SETUP ===
echo "════════════════════════════════════════════════════════════"
echo "                RustOS Dev Loop (llama.cpp)"
echo "════════════════════════════════════════════════════════════"
log "INFO" "========================================"
log "INFO" "dev_llama.sh started"
log "INFO" "========================================"

# Kill any existing llama-server/aider processes
...
for i in {1..10}; do
    LIVE_LLAMA=$(pgrep -f "llama-server" | wc -l)
    if [ "$LIVE_LLAMA" -eq 0 ]; then
        break
    fi
    echo "Waiting for $LIVE_LLAMA llama-server process(es) to terminate..."
    pkill -9 -f "llama-server" 2>/dev/null
    sleep 1
done
...
log "INFO" "Starting llama.cpp server on port 8000"
...
for attempt in {1..60}; do
    if curl -s "$LLAMA_HOST/v1/models" > /dev/null 2>&1; then
        ...
        log "INFO" "llama.cpp server ready after $attempt seconds"
        break
    fi
    if [ $attempt -eq 60 ]; then
        echo "✗ llama.cpp server failed to start after 60 seconds"
...
while true; do
    ...
    if [ $SAME_TASK_COUNT -ge 2 ]; then
        ...
        CONTEXT_MSG="The local AI (aider with Qwen2.5-Coder 7B on llama.cpp) is stuck on this RustOS project."
...
"
    ...
    # llama.cpp with Qwen2.5-Coder-7B GGUF - OpenAI-compatible API
